...
The mailbox can now orchestrate event, timer, and conversational-review turns automatically. Each wake runs one agent Activity followed by the idempotent action-persistence Activity, records the result, and installs the validated wake plan when no actions remain unresolved. Turns are single-flight while Signals continue buffering. Proposed action IDs remain visibly pending until execution or review resolves them.

The first provider-neutral execution stage is also available with a deterministic stub adapter. It writes an action attempt before calling the provider, uses a stable payload-bound idempotency key, revalidates exact human approval immediately before dispatch, and records definitive failure separately from an ambiguous outcome. Retrying an ambiguous attempt performs provider lookup first; the test suite covers timeout-after-success recovery without a duplicate provider call. The workflow executes autonomously allowed actions immediately, defers approval-required actions, and installs the agent's wake plan only after all actions succeed. Failed or unknown actions remain pending for the forthcoming reconciliation path.
