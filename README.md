...
Rejected turns and exhausted automatic chains enter a durable intervention state instead of disappearing inside workflow history. Tenant-scoped operators can issue attributed, idempotent retry, wake, and takeover controls through the API; control Signals return to the same logical mailbox and retry preserves the failed turn's bounded source lineage. The Vue console currently displays these records in the process timeline, while dedicated intervention controls remain future UI work.
