...
Compose deployments through an explicit versioned extension manifest at worker startup. Private packs pin Tiramisu and pass public contracts. They may narrow authority but cannot bypass core isolation or safety. Do not store canonical client work only in a gitignored directory.
...
Public CI has no private dependency. Local client development uses sibling editable packages; CI and deployments use immutable revisions or artifacts. MIT permits commercial private forks and hosted derivatives.
