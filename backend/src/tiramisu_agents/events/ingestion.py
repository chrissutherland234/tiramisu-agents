        ...
        *,
        ...
        if bootstrap is not None:
            if not event.external_references:
                raise ValueError("a process-triggering event requires an external reference")
        ...
        if process_id is None and status == "pending" and bootstrap is not None:
            process_id = await self._create_process(session, event, bootstrap)
            status = "matched"
    ...
    async def _resolve_process(
        ...
        if event.external_references:
            predicates = [
                ...
                for reference in event.external_references
            ]
            candidates.update(
                (
    ...
    async def _create_process(
        ...
        session.add(
            ProcessInstance(
                ...
                status="active",
                workflow_id=f"tenant/{event.tenant_id}/process/{process_id}",
            )
        )
        await session.flush()
        session.add_all(
